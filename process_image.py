...
# 1) Data Structures
class FrameInfo:
    def __init__(self, camera_index, frame_index, camera_position, yaw, pitch, roll, fov_degrees, image_file):
...
def detect_motion(prev, curr, threshold=2.0):
...
def process_all(metadata_path, images_folder, output_bin):
    ...
    for cam_id, cam_frames in frames_by_cam.items():
        ...
        for i, curr_info in enumerate(cam_frames):
            ...
            curr_img = load_image_gray(img_path)
            if prev_img is None:
                prev_img = curr_img
                continue
...
# Example usage:
# process_all('metadata.json', 'images_folder', 'output_voxel_grid.bin')